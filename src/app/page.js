    ...
    <main className="min-h-screen bg-[#f3f6fb] text-slate-950">
      <div className="mx-auto flex min-h-screen w-full max-w-[1680px]">
        <aside className="hidden w-64 flex-col justify-between border-r border-slate-200/70 bg-slate-950 p-5 text-white shadow-[0_20px_80px_rgba(15,23,42,0.18)] lg:flex">
          ...
          <button onClick={() => setUsuario(null)} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-left transition hover:bg-white/10"><span className="grid h-11 w-11 place-items-center rounded-xl bg-white text-sm font-black text-slate-950">{usuario.iniciais}</span><span><span className="block text-sm font-black text-white">{usuario.nome}</span><span className="block text-xs text-slate-400">Sair da demo</span></span></button>
        </aside>
...
function Dashboard({ ehCliente, obraAtual, usuario, resumo, alertas, atrasosReais, possiveisAtrasos, fotosDaObra, materiaisRecebidosHoje, tarefas, obrasVisiveis, cronogramas, setObraId, setCardDetalhe, financeiro, compras, atualizarObra }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="overflow-hidden rounded-[2.5rem] bg-slate-950 text-white shadow-[0_30px_90px_rgba(15,23,42,0.25)]">
      <div className="relative isolate p-7 lg:p-9">
        ...
        <div className="relative z-10 grid gap-8 xl:grid-cols-[1.25fr_.75fr]">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-300">{obraAtual.cliente} · {obraAtual.endereco}. Etapa atual: <strong className="text-white">{obraAtual.etapa}</strong>.</p>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-white/10 p-6 backdrop-blur-xl">
            ...
            <div className="mt-6 flex items-center justify-center"><ProgressRingDark value={resumo.media} /></div>
            <div className="mt-6 grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-2xl bg-white/10 p-4"><p className="text-slate-300">Atrasos</p><p className="mt-1 text-2xl font-black text-red-200">{tarefasAtrasadas}</p></div>
              <div className="rounded-2xl bg-white/10 p-4"><p className="text-slate-300">Compras abertas</p><p className="mt-1 text-2xl font-black text-amber-200">{comprasAbertas}</p></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="grid gap-6 xl:grid-cols-[1fr_.72fr]">
      ...
      <PanelClean>
        <div className="mb-4"><p className={eyebrowClass}>Portfólio</p><h2 className="mt-1 text-2xl font-black text-slate-950">Obras em acompanhamento</h2></div>
        <div className="space-y-3">{obrasVisiveis.map((obra) => { const tarefasObra = cronogramas[obra.id] || []; const p = tarefasObra.length ? Math.round(tarefasObra.reduce((a, t) => a + Number(t.progresso), 0) / tarefasObra.length) : 0; return <button key={obra.id} onClick={() => setObraId(obra.id)} className="w-full rounded-[1.5rem] border border-slate-200/70 bg-white/80 p-4 text-left transition hover:-translate-y-0.5 hover:bg-blue-50 hover:shadow-lg"><div className="flex items-center justify-between gap-3"><div><p className="font-black text-slate-950">{obra.nome}</p><p className="mt-1 text-xs text-slate-500">{obra.etapa} · {obra.prazo}</p><p className="mt-1 text-xs font-bold text-blue-700">Responsável: {obra.responsavel}</p></div><div className="text-right"><p className="text-2xl font-black text-blue-700">{p}%</p><p className="text-[10px] uppercase tracking-widest text-slate-400">avanço</p></div></div></button> })}</div>
      </PanelClean>
    </section>
...
function PanelClean({ children }) { return <section className="rounded-[2.25rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-slate-200/60 backdrop-blur-xl transition hover:shadow-2xl hover:shadow-slate-200/70">{children}</section> }
function MenuItem({ label, text, active, onClick }) { return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition ${active ? 'bg-white text-slate-950 shadow-xl shadow-black/20' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}><span className={`grid h-9 w-9 place-items-center rounded-xl text-lg ${active ? 'bg-slate-950 text-white' : 'bg-white/10 text-white'}`}>{label}</span><span className="text-sm font-black">{text}</span></button> }
...
function ClienteBanner({ obraAtual }) { return <div className="rounded-[2rem] border border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50 p-5 shadow-sm"><p className="font-black text-blue-800">Painel do cliente</p><p className="mt-1 text-sm text-blue-700">Você está visualizando somente a obra vinculada ao seu acesso: <strong>{obraAtual.nome}</strong>.</p></div> }
function InfoCard({ titulo, valor, detalhe }) { return <div className="rounded-[2rem] border border-slate-200/70 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm"><p className="text-xs font-bold uppercase tracking-[0.25em] text-slate-400">{titulo}</p><p className="mt-2 text-xl font-black">{valor}</p><p className="mt-1 text-sm text-slate-500">{detalhe}</p></div> }
function MiniTimeline({ tarefas }) { return <div className="rounded-[2rem] border border-slate-200/70 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[0.25em] text-slate-400">Cronograma</p><p className="text-xs font-bold text-emerald-600">Atualizado hoje</p></div>{tarefas.length === 0 ? <p className="text-sm text-slate-500">Nenhum cronograma criado ainda.</p> : tarefas.slice(0, 5).map((item) => <div key={item.id} className="mb-3 last:mb-0"><div className="mb-1 flex justify-between text-xs text-slate-500"><span>{item.nome}</span><span>{item.progresso}%</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-emerald-400" style={{ width: `${item.progresso}%` }} /></div></div>)}</div> }
function MetricCard({ title, value, detail, icon, onClick }) { return <button onClick={onClick} className="group relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 p-6 text-left shadow-xl shadow-slate-200/60 backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-2xl"><div className="absolute inset-0 bg-gradient-to-br from-blue-50/60 via-transparent to-emerald-50/40 opacity-0 transition group-hover:opacity-100" /><div className="relative z-10 flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-slate-500">{title}</p><p className="mt-3 text-4xl font-black tracking-[-0.04em] text-slate-950">{value}</p><p className="mt-3 text-sm text-slate-400">{detail}</p></div><div className={professionalIconClass}>{icon}</div></div></button> }
function ExecutiveStat({ label, value, detail }) { return <div className="rounded-[1.5rem] border border-white/10 bg-white/10 p-4 backdrop-blur-xl"><p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-300">{label}</p><p className="mt-2 truncate text-2xl font-black text-white">{value}</p><p className="mt-1 text-xs text-slate-400">{detail}</p></div> }
function ProgressRingDark({ value }) { const radius = 38; const circumference = 2 * Math.PI * radius; const offset = circumference - (value / 100) * circumference; return <div className="relative grid h-32 w-32 place-items-center"><svg className="h-32 w-32 -rotate-90" viewBox="0 0 100 100"><circle cx="50" cy="50" r={radius} stroke="rgba(255,255,255,.12)" strokeWidth="10" fill="none" /><circle cx="50" cy="50" r={radius} stroke="url(#neoDarkGradient)" strokeWidth="10" fill="none" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} /><defs><linearGradient id="neoDarkGradient" x1="0" x2="1" y1="0" y2="1"><stop offset="0%" stopColor="#34d399" /><stop offset="55%" stopColor="#60a5fa" /><stop offset="100%" stopColor="#93c5fd" /></linearGradient></defs></svg><div className="absolute text-center"><p className="text-4xl font-black text-white">{value}%</p><p className="text-[10px] uppercase tracking-widest text-slate-400">obra</p></div></div> }
