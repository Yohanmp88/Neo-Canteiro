...
const buttonGreenClass = 'rounded-2xl bg-gradient-to-r from-emerald-400 to-teal-400 px-5 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-emerald-950/40 ring-1 ring-white/10 transition hover:-translate-y-0.5 hover:shadow-emerald-900/50'
...
const professionalIconClass = 'grid h-11 w-11 place-items-center rounded-2xl bg-gradient-to-br from-cyan-400 to-blue-500 text-sm font-bold text-slate-950 shadow-lg shadow-cyan-950/40 ring-1 ring-white/20'
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.18),transparent_34%),linear-gradient(135deg,#020617_0%,#050816_45%,#020617_100%)] text-slate-100">
      <div className="mx-auto flex min-h-screen w-full max-w-[1680px]">
        <aside className="hidden w-64 flex-col justify-between border-r border-white/10 bg-slate-950/90 p-5 text-white shadow-[0_24px_90px_rgba(0,0,0,0.55)] backdrop-blur-2xl lg:flex">
          ...
          <button onClick={() => setUsuario(null)} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-left transition hover:bg-white/10"><span className="grid h-11 w-11 place-items-center rounded-xl bg-white text-sm font-bold text-slate-950">{usuario.iniciais}</span><span><span className="block text-sm font-black text-white">{usuario.nome}</span><span className="block text-xs text-slate-400">Sair da demo</span></span></button>
        </aside>
...
function Dashboard({ ehCliente, obraAtual, usuario, resumo, alertas, atrasosReais, possiveisAtrasos, fotosDaObra, materiaisRecebidosHoje, tarefas, obrasVisiveis, cronogramas, setObraId, setCardDetalhe, financeiro, compras, atualizarObra }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/95 text-white shadow-[0_18px_54px_rgba(0,0,0,0.32)]">
      <div className="relative isolate p-5 lg:p-7">
        <div className="absolute -right-28 -top-28 h-56 w-56 rounded-full bg-blue-500/12 blur-3xl" />
        <div className="absolute -bottom-32 left-1/3 h-56 w-56 rounded-full bg-emerald-400/8 blur-3xl" />
        <div className="relative z-10 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
          <div>
            <div className="flex flex-wrap items-center gap-3">
              <span className="rounded-full bg-white/8 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-blue-100 ring-1 ring-white/10">Controle executivo da obra</span>
              <StatusBadge status={obraAtual.status} />
            </div>
            <h1 className="mt-5 max-w-3xl text-4xl font-bold tracking-[-0.035em] text-white lg:text-5xl">{obraAtual.nome}</h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">{obraAtual.cliente} · {obraAtual.endereco}. Etapa atual: <strong className="text-white">{obraAtual.etapa}</strong>.</p>
          ...
          <div className="rounded-[1.5rem] border border-white/10 bg-white/[0.07] p-5 backdrop-blur-xl">
            <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-300">Saúde operacional</p>
            <div className="mt-4 flex items-center justify-center"><ProgressRingDark value={resumo.media} /></div>
            <div className="mt-4 grid grid-cols-2 gap-3 text-xs">
              <div className="rounded-2xl bg-white/[0.07] p-3"><p className="text-slate-300">Atrasos</p><p className="mt-1 text-xl font-bold text-red-200">{tarefasAtrasadas}</p></div>
              <div className="rounded-2xl bg-white/[0.07] p-3"><p className="text-slate-300">Compras abertas</p><p className="mt-1 text-xl font-bold text-amber-200">{comprasAbertas}</p></div>
            </div>
          </div>
        </div>
      </div>
    ...
    {!ehCliente && <PanelClean><div className="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between"><div><p className={eyebrowClass}>Planejamento</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Cronograma executivo</h2></div><p className="text-sm text-slate-500">Previsto x realizado, atualizado automaticamente</p></div><GraficoCronograma tarefas={tarefas} /></PanelClean>}

    {ehCliente && <PanelClean>
      <div className="mb-4 flex items-center justify-between"><div><p className={eyebrowClass}>Fase atual</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Etapa e cronograma simplificado</h2></div><StatusBadge status={obraAtual.status} /></div>
...
function TelaTemplates() { return <div className="space-y-6"><section className="overflow-hidden rounded-[2.5rem] bg-slate-950 p-8 text-white shadow-[0_30px_90px_rgba(15,23,42,0.22)]"><p className="text-xs font-black uppercase tracking-[0.28em] text-blue-200">Biblioteca profissional</p><h1 className="mt-3 text-5xl font-black tracking-[-0.04em]">Templates para obra</h1><p className="mt-3 max-w-2xl text-slate-300">Modelos prontos para padronizar relatórios, medições, diários e solicitações dentro do NeoCanteiro.</p></section><div className="grid gap-5 md:grid-cols-3">{templates.map((t, i) => <div key={t} className="group rounded-[2rem] border border-white/70 bg-white/90 p-6 shadow-xl shadow-slate-200/60 transition hover:-translate-y-1 hover:shadow-2xl"><div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-[0.22em] text-blue-700">{i === 3 || i === 4 ? 'Popular' : 'Template'}</p><span className="grid h-10 w-10 place-items-center rounded-2xl bg-slate-950 text-xs font-black text-white">0{i + 1}</span></div><h3 className="mt-6 text-2xl font-black tracking-tight text-slate-950">{t}</h3><p className="mt-3 text-sm leading-relaxed text-slate-500">Modelo padronizado para agilizar rotinas de campo, controle técnico e comunicação com cliente.</p><button className="mt-6 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 font-black text-slate-700 transition group-hover:bg-slate-950 group-hover:text-white">Usar modelo</button></div>)}</div></div> }
...
function LoginScreen({ selecionarUsuario }) {
  ...
  if (autenticado) {
    return (
      <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dbeafe_0,#f8fafc_38%,#eef2ff_100%)] p-5 text-slate-950">
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dbeafe_0,#f8fafc_38%,#eef2ff_100%)] p-5 text-slate-950">
      <div className="relative mx-auto grid min-h-screen max-w-5xl place-items-center">
        <div className="grid w-full overflow-hidden rounded-[2.5rem] border border-white/70 bg-white/90 shadow-2xl shadow-slate-300/60 backdrop-blur-xl lg:grid-cols-[1.05fr_.95fr]">
          <section className="relative overflow-hidden bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 p-8 text-white lg:p-12">
            <LogoNeoCanteiro />
            <div className="mt-16">
...
function LogoDark() { return <div className="flex items-center gap-3"><img src="/logo-neocanteiro.png" alt="NeoCanteiro" className="h-12 w-12 rounded-2xl bg-white object-cover" /><div><p className="text-lg font-black tracking-tight text-white">NeoCanteiro</p><p className="text-[10px] uppercase tracking-[0.28em] text-slate-400">Construction OS</p></div></div> }
function PanelClean({ children }) { return <section className="rounded-[1.5rem] border border-white/60 bg-white/[0.86] p-5 shadow-[0_14px_42px_rgba(0,0,0,0.18)] backdrop-blur-xl transition hover:shadow-[0_18px_48px_rgba(0,0,0,0.22)]">{children}</section> }
function MenuItem({ label, text, active, onClick }) { return <button onClick={onClick} className={`flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-left transition ${active ? 'bg-white text-slate-950 shadow-md shadow-black/20' : 'text-slate-300 hover:bg-white/8 hover:text-white'}`}><span className={`grid h-8 w-8 place-items-center rounded-lg text-sm ${active ? 'bg-slate-950 text-white' : 'bg-white/8 text-white'}`}>{label}</span><span className="text-[13px] font-semibold">{text}</span></button> }
...
function ClienteBanner({ obraAtual }) { return <div className="rounded-[1.35rem] border border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50 p-4 shadow-sm"><p className="text-sm font-bold text-blue-800">Painel do cliente</p><p className="mt-1 text-xs text-blue-700">Você está visualizando somente a obra vinculada ao seu acesso: <strong>{obraAtual.nome}</strong>.</p></div> }
function InfoCard({ titulo, valor, detalhe }) { return <div className="rounded-[1.15rem] border border-slate-200/70 bg-gradient-to-br from-slate-50 to-white p-4 shadow-sm"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">{titulo}</p><p className="mt-1.5 text-base font-bold">{valor}</p><p className="mt-1 text-xs text-slate-500">{detalhe}</p></div> }
function MiniTimeline({ tarefas }) { return <div className="rounded-[1.15rem] border border-slate-200/70 bg-gradient-to-br from-slate-50 to-white p-4 shadow-sm"><div className="mb-3 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400">Cronograma</p><p className="text-[10px] font-bold text-emerald-600">Atualizado hoje</p></div>{tarefas.length === 0 ? <p className="text-xs text-slate-500">Nenhum cronograma criado ainda.</p> : tarefas.slice(0, 5).map((item) => <div key={item.id} className="mb-2.5 last:mb-0"><div className="mb-1 flex justify-between text-xs text-slate-500"><span>{item.nome}</span><span>{item.progresso}%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-emerald-400" style={{ width: `${item.progresso}%` }} /></div></div>)}</div> }
function MetricCard({ title, value, detail, icon, onClick }) { return <button onClick={onClick} className="group relative overflow-hidden rounded-[1.35rem] border border-white/60 bg-white/[0.86] p-5 text-left shadow-[0_12px_36px_rgba(0,0,0,0.16)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:shadow-[0_18px_44px_rgba(0,0,0,0.2)]"><div className="absolute inset-0 bg-gradient-to-br from-blue-50/35 via-transparent to-emerald-50/20 opacity-0 transition group-hover:opacity-100" /><div className="relative z-10 flex items-start justify-between gap-3"><div><p className="text-xs font-semibold text-slate-500">{title}</p><p className="mt-2 text-3xl font-bold tracking-[-0.035em] text-slate-950">{value}</p><p className="mt-2 text-xs text-slate-400">{detail}</p></div><div className={professionalIconClass}>{icon}</div></div></button> }
function ExecutiveStat({ label, value, detail }) { return <div className="rounded-[1.1rem] border border-white/10 bg-white/[0.07] p-3.5 backdrop-blur-xl"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-300">{label}</p><p className="mt-1.5 truncate text-lg font-bold text-white">{value}</p><p className="mt-0.5 text-xs text-slate-400">{detail}</p></div> }
function ProgressRingDark({ value }) { const radius = 38; const circumference = 2 * Math.PI * radius; const offset = circumference - (value / 100) * circumference; return <div className="relative grid h-28 w-28 place-items-center"><svg className="h-28 w-28 -rotate-90" viewBox="0 0 100 100"><circle cx="50" cy="50" r={radius} stroke="rgba(255,255,255,.12)" strokeWidth="10" fill="none" /><circle cx="50" cy="50" r={radius} stroke="url(#neoDarkGradient)" strokeWidth="10" fill="none" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} /><defs><linearGradient id="neoDarkGradient" x1="0" x2="1" y1="0" y2="1"><stop offset="0%" stopColor="#34d399" /><stop offset="55%" stopColor="#60a5fa" /><stop offset="100%" stopColor="#93c5fd" /></linearGradient></defs></svg><div className="absolute text-center"><p className="text-3xl font-bold text-white">{value}%</p><p className="text-[10px] uppercase tracking-widest text-slate-400">obra</p></div></div> }
...
function UserCard({ tipo, texto }) { return <div className="rounded-[1.35rem] border border-slate-200 bg-slate-50 p-5"><h3 className="text-xl font-bold">{tipo}</h3><p className="mt-2 text-sm leading-relaxed text-slate-500">{texto}</p></div> }
function Empty({ text }) { return <div className="rounded-[1.35rem] border border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">{text}</div> }
