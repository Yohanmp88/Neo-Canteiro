...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#dbeafe_0,#f8fafc_34%,#eef2ff_100%)] text-slate-950">
      <div className="mx-auto flex min-h-screen w-full max-w-[1500px]">
        <aside className="hidden w-56 flex-col justify-between border-r border-slate-200/70 bg-white/75 p-5 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur-2xl lg:flex">
        ...
        <section className="flex-1 overflow-hidden px-4 py-5 lg:px-8">
          <header className="mb-6 flex flex-col gap-5 rounded-[2rem] border border-white/70 bg-white/80 p-5 shadow-[0_20px_70px_rgba(15,23,42,0.08)] backdrop-blur-2xl xl:flex-row xl:items-center xl:justify-between"><LogoNeoCanteiro /><div className="flex flex-wrap items-center gap-3">{obrasVisiveis.length > 1 && <select value={obraId} onChange={(e) => setObraId(Number(e.target.value))} className={`${inputClass} min-w-60`}>{obrasVisiveis.map((obra) => <option key={obra.id} value={obra.id}>{obra.nome}</option>)}</select>}{permissaoAdmin && <button onClick={() => trocarTela('usuarios')} className={buttonPrimaryClass}>Nova obra</button>}</div></header>
...
function MenuItem({ label, text, active, onClick }) { return <button onClick={onClick} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition ${active ? 'bg-gradient-to-r from-slate-950 to-blue-950 text-white shadow-lg shadow-blue-950/20' : 'bg-white/70 text-slate-600 ring-1 ring-slate-200/70 hover:bg-blue-50 hover:text-blue-900'}`}><span className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-lg">{label}</span><span className="text-sm font-black">{text}</span></button> }
...
function MetricCard({ title, value, detail, icon, onClick }) { return <button onClick={onClick} className="group relative overflow-hidden rounded-[2rem] border border-white/70 bg-white/90 p-6 text-left shadow-xl shadow-slate-200/60 backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-2xl"><div className="absolute inset-0 bg-gradient-to-br from-blue-50/40 via-transparent to-emerald-50/30 opacity-0 transition group-hover:opacity-100" /><div className="relative z-10 flex items-start justify-between gap-4"><div><p className="text-sm font-medium text-slate-500">{title}</p><p className="mt-3 text-4xl font-black tracking-tight text-slate-950">{value}</p><p className="mt-3 text-sm text-slate-400">{detail}</p></div><div className="grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-slate-100 to-blue-100 text-2xl shadow-inner">{icon}</div></div></button> }
